...
#include <CocktailEngine/Core/Math/Vector/Vector2.hpp>
...
namespace Ck::ImUi
{
    WindowRenderer::WindowRenderer(Renderer::RenderDevice* renderDevice)
    {
        ...
        shaderProgramCreateInfo.Shaders[0] = loadShader(Renderer::ShaderType::Vertex, CK_TEXT("builtin://imguiintegration/resources/default.vert.spv"));
        shaderProgramCreateInfo.Shaders[1] = loadShader(Renderer::ShaderType::Fragment, CK_TEXT("builtin://imguiintegration/resources/default.frag.spv"));
    ...
    void WindowRenderer::Draw(ImGuiContext* context, WindowResources* windowResources, Renderer::RenderContext* renderContext, Renderer::Framebuffer* framebuffer) const
    {
        ...
        for (ImDrawList* drawList : drawData.CmdLists)
        {
            ...
            commandList->BindVertexBuffer(0, vertexBufferArea.BufferResource, vertexBufferArea.BaseOffset, sizeof(ImDrawVert));
            commandList->BindIndexBuffer(indexBufferArea.BufferResource, indexBufferArea.BaseOffset, sizeof(ImDrawIdx) == 2 ? Renderer::IndexType::Short : Renderer::IndexType::Integer);
