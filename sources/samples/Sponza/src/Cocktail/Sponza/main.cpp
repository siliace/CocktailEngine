...
Main::ExitCode ApplicationMain(Application* application)
{
	...
	// Closing the window and asking the application to exit are the same request, so
	// they take the same path. The loop below then has a single condition to read, and
	// it reads the same on every platform.
	application->Connect(window->OnCloseEvent(), [&](const WindowCloseEvent&) {
		application->Exit(0, CK_TEXT("window closed"));
	});

	Duration lastFrameBegin = application->Uptime();
	while (!application->IsExitRequested())
	{
		// A window can also go away without asking, and its liveness is the one thing
		// the pump answers. Routed into the same request, so that it stops the loop the
		// way a close does instead of leaving it to render against a dead handle.
		if (!window->PollEvents())
			application->Exit(0, CK_TEXT("window destroyed"));

		// Pumping the events is what turns a close into an exit request, so the request
		// is read again before rendering: the window may be gone already.
