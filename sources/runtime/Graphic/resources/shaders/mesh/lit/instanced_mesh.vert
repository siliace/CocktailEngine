...
struct InstanceTransformationInfo
{
	mat4 modelMatrix;
	mat3 normalMatrix;
};
...
layout (set = 3, binding = 0) readonly buffer InstancingInfo {
	InstanceTransformationInfo transformations[];
} instances;
...
#ifdef COCKTAIL_VERTEX_HAS_NORMAL
	mat3 normalMatrix = instances.transformations[gl_InstanceIndex].normalMatrix * vertexInfo.normalMatrix;
	normal = normalMatrix * ck_VertexNormal;

    #ifdef COCKTAIL_VERTEX_HAS_TANGENT
    tangent = normalize(normalMatrix * ck_VertexTangent);
    biTangent = normalize(normalMatrix * ck_VertexBiTangent);
    #endif
#endif

	vec4 world = instances.transformations[gl_InstanceIndex].modelMatrix * vertexInfo.modelMatrix * vec4(ck_VertexPosition, 1.f);
