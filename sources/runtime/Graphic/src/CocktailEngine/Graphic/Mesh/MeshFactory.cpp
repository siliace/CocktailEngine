...
#include <CocktailEngine/Graphic/Geometry/Index/IndexRef.hpp>
#include <CocktailEngine/Graphic/Geometry/Vertex/VertexArray.hpp>
#include <CocktailEngine/Graphic/Geometry/Vertex/VertexLayout.hpp>
...
namespace Ck
{
	SharedPtr<Mesh> MeshFactory::CreateCube(float size, const LinearColor& color)
	{
	...
	SharedPtr<Mesh> MeshFactory::CreateBox(const Box<float>& box, const LinearColor& color)
	{
		SharedPtr<VertexLayout> vertexLayout = VertexLayout::Builder()
		...
		SharedPtr<VertexArray> vertexArray = MakeShared<VertexArray>(Move(vertexLayout), 24);
		...
		SharedPtr<IndexArray> indexArray = MakeShared<IndexArray>(Renderer::IndexType::Short, 36);
		...
		return MakeShared<Mesh>(Move(vertexArray), Move(indexArray));
	}
}
