#include <CocktailEngine/Graphic/Scene/Container/Gltf/GltfSceneContainer.hpp>
...
namespace Ck
{
    GltfSceneContainer::GltfSceneContainer(const tinygltf::Model& model)
    {
        for (const tinygltf::Image& gltfImage: model.images)
        {
            ...
            Image image(size, imageRawFormat, GammaSpace::Linear, Move(pixels));
        ...
        {
            childrenNodeInfo.Add(ProcessNode(model, model.nodes[sceneNodeIndex]));
        }

        mRoot.Children = Move(childrenNodeInfo);
    ...
    SceneContainer::MaterialInfo GltfSceneContainer::ProcessMaterial(const tinygltf::Model& model, const tinygltf::Material& gltfMaterial) const
    {
        ...
        {
            const tinygltf::Texture& texture = model.textures[textureIndex];
            materialInfo.Textures[Material::TextureType::Normal] = mMipMaps[texture.source];
        }
    ...
    void GltfSceneContainer::ProcessMesh(const tinygltf::Model& model, unsigned int meshIndex, const tinygltf::Mesh& gltfMesh)
    {
        ...
        for (const tinygltf::Primitive& primitive: gltfMesh.primitives)
        {
            ...
            if (vertexLayout->FindAttribute(VertexAttributeSemantic::Normal) && vertexLayout->FindAttribute(VertexAttributeSemantic::Tangent))
            {
                for (std::size_t i = 0; i < vertexCount; i++)
                {
                    VertexRef vertex = vertices->At(i);
                    Vector3<float> normal = vertex.GetVector<Vector3<float>>(VertexAttributeSemantic::Normal);
                    Vector4<float> tangent = vertex.GetVector<Vector4<float>>(VertexAttributeSemantic::Tangent);

                    Vector3<float> biTangent = Vector3<float>::CrossProduct(normal, tangent.XYZ()) * tangent.W();
                    vertex.SetVector(VertexAttributeSemantic::BiTangent, biTangent);
                }
            }
            ...
            else
            {
                ...
                meshInfo.Vertices = Move(vertices);
                meshInfo.Indices = Move(indices);
        ...
        {
            mMeshes.Add(Move(primitiveMesh));
    ...
    SceneContainer::NodeInfo GltfSceneContainer::ProcessNode(const tinygltf::Model& model, const tinygltf::Node& gltfNode)
    {
        ...
        {
            childrenNodes.Add(ProcessNode(model, model.nodes[childSceneNodeIndex]));
        }
        ...
        NodeInfo nodeInfo;
        nodeInfo.Children = Move(childrenNodes);
