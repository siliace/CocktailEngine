...
#include <CocktailEngine/Graphic/Material/Shading/MaterialProgram.hpp>
...
namespace Ck
{
        ...
        mShadingMode(createInfo.ShadingMode)
	{
		for (const EnumMap<Renderer::ShaderType, ByteArray>& variantBinaries : createInfo.VariantsBinaries)
		{
			...
			UniquePtr<MaterialProgramVariant> variant = MakeUnique<MaterialProgramVariant>(createInfo.Interface, Move(shaderProgram));
			...
			mVariants.Put(VariantKey(vertexAttributes, materialTextures), Move(variant));
		}
	}
