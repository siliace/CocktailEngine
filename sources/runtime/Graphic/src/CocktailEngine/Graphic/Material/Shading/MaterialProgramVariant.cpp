...
namespace Ck
{
    ...
    void BindingDomainInterface::SetSlot(BindingSlot bindingSlot, AsciiString name, Renderer::DescriptorType type, Renderer::StaticSampler sampler)
    {
        ...
        auto createBindingSlotInterface = [&]() -> SlotInterface& {
            ...
            bindingSlotInterface.Slot = bindingSlot;
    ...
    MaterialProgramVariant::MaterialProgramVariant(SharedPtr<MaterialProgramInterface> interface, SharedPtr<Renderer::ShaderProgram> shaderProgram) :
        mInterface(Move(interface)),
        mShaderProgram(Move(shaderProgram))
    {
