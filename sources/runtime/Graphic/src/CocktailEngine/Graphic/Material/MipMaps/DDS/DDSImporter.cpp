...
#include <CocktailEngine/Graphic/Material/MipMaps/CubeMipMaps.hpp>
#include <CocktailEngine/Graphic/Material/MipMaps/DDS/DDSHeader.hpp>
#include <CocktailEngine/Graphic/Material/MipMaps/DDS/DDSHeaderDxt10.hpp>
...
namespace Ck
{
    ...
    namespace
    {
        ...
        {
            const bool hasAlpha = pixelFormat.FormatFlags & DDSPixelFormat::FlagBits::AlphaPixels || pixelFormat.FormatFlags & DDSPixelFormat::FlagBits::Alpha;
            ...
            if (pixelFormat.FormatFlags & DDSPixelFormat::FlagBits::FourCC)
            {
    ...
    SharedPtr<MipMaps> DDSImporter::LoadFromStream(InputStream<>& inputStream, const MipMapsImportParameters& parameters)
    {
        ...
        if (header.PixelFormat.Size != sizeof(DDSPixelFormat) || header.PixelFormat.FormatFlags & DDSPixelFormat::FlagBits::YUV ||
            header.PixelFormat.FormatFlags & DDSPixelFormat::FlagBits::Luminance)
            throw DDSImportException(CK_TEXT("Invalid DDS PixelFormat"));

        Optional<DDSHeaderDxt10> dxt10Header;
        if (header.PixelFormat.FormatFlags & DDSPixelFormat::FlagBits::FourCC && header.PixelFormat.FourCC == MakeFourCC('D', 'X', '1', '0'))
        {
