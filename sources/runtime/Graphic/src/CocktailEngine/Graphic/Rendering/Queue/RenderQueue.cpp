...
#include <CocktailEngine/Graphic/Rendering/Queue/CustomRecord.hpp>
#include <CocktailEngine/Graphic/Rendering/Queue/LineRecord.hpp>
#include <CocktailEngine/Graphic/Rendering/Queue/RenderQueue.hpp>
#include <CocktailEngine/Graphic/Rendering/Queue/StaticMeshRecord.hpp>
...
namespace Ck
{
    namespace
    {
        Flags<VertexAttributeSemantic> GetStaticMeshRecordVertexAttributes(const StaticMeshRecordInfo& recordInfo)
        {
            ...
            for (unsigned int i = 0; i < recordInfo.VertexBufferCount; i++)
            {
                ...
                {
                    if (!recordInfo.VertexBuffers[i].Layout->FindAttribute(attribute))
                        continue;
        ...
        Flags<Material::TextureType> GetStaticMeshRecordMaterialTextures(const StaticMeshRecordInfo& recordInfo)
        {
    ...
    {
        Flags<VertexAttributeSemantic> vertexAttributes = GetStaticMeshRecordVertexAttributes(recordInfo);
        Flags<Material::TextureType> materialTextures = GetStaticMeshRecordMaterialTextures(recordInfo);

        assert(vertexAttributes & VertexAttributeSemantic::Position);
    ...
    void RenderQueue::PushInstancedStaticMesh(const InstancedStaticMeshRecordInfo& recordInfo, Material::ShadingMode shadingMode, Uint64 sortingKey)
    {
        Flags<VertexAttributeSemantic> vertexAttributes = GetStaticMeshRecordVertexAttributes(recordInfo.StaticMeshRecord);
        Flags<Material::TextureType> materialTextures = GetStaticMeshRecordMaterialTextures(recordInfo.StaticMeshRecord);

        assert(vertexAttributes & VertexAttributeSemantic::Position);

        if (MaterialProgramVariant* variant = FindMaterialProgramVariant(RenderableType::InstancedMesh, shadingMode, vertexAttributes, materialTextures))
            Emplace(InstancedStaticMeshRecord::New(recordInfo, variant), sortingKey);
    }
    ...
    {
        mRecords.Emplace(RecordInfo{ Move(record), sortingKey });
    }

    MaterialProgramVariant* RenderQueue::FindMaterialProgramVariant(RenderableType renderable, Material::ShadingMode shadingMode, Flags<VertexAttributeSemantic> vertexAttributes,
                                                                    Flags<Material::TextureType> materialTextures) const
    {
        ...
        {
            CK_LOG(RenderQueueLogCategory, LogLevel::Error, CK_TEXT("No variant found in material program %s with usage mask %d and %d"), materialProgram->GetName(),
                   vertexAttributes, materialTextures);
            return nullptr;
        }
