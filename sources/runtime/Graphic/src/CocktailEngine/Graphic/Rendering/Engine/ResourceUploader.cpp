#include <CocktailEngine/Graphic/Rendering/Engine/ResourceUploader.hpp>

namespace Ck
{
	void ResourceUploader::RequestBufferUpload(SharedPtr<BufferResource> buffer, std::size_t offset, std::size_t length, const void* data)
	{
		...
		bufferUploadRequest.UploadInfo.Add({ offset, length, data });
		mBufferUploadRequests.Add(Move(bufferUploadRequest));
	}

	void ResourceUploader::RequestTextureUpload(SharedPtr<TextureResource> texture, unsigned int arrayLayer, unsigned int mipMapLevel, const void* data)
	{
		...
		textureUploadRequest.UploadInfo.Add({ arrayLayer, mipMapLevel, uploadSize, MakeExtent(0u, 0u, 0u), data });
		mTextureUploadRequests.Add(Move(textureUploadRequest));
	}
