...
#include <CocktailEngine/Graphic/Rendering/Resource/MipMapsTextureResource.hpp>
...
namespace Ck
{
    ...
    {
		SharedPtr<VertexBuffer> vertexBuffer = MakeShared<VertexBuffer>(AsShared(), Move(vertices), name);
		vertexBuffer->Upload();
	...
	{
		SharedPtr<IndexBuffer> indexBuffer = MakeShared<IndexBuffer>(AsShared(), Move(indices), name);
		indexBuffer->Upload();
