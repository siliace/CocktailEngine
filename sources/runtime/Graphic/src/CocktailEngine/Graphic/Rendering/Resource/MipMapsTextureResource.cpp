...
namespace Ck
{
	MipMapsTextureResource::MipMapsTextureResource(SharedPtr<GraphicEngine> graphicEngine, SharedPtr<MipMaps> mipMaps, const AnsiChar* name) :
		TextureResource(Move(graphicEngine), mipMaps->GetBaseSize(), mipMaps->GetPixelFormat(), mipMaps->GetArrayLayerCount(), mipMaps->GetMipMapCount(), mipMaps->IsCube(), name),
		mMipMaps(Move(mipMaps))
	{
		/// Nothing
	}
