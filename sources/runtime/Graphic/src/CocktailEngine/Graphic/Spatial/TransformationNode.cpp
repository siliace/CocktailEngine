...
namespace Ck
{
	TransformationNode::TransformationNode(Transformation localTransformation) :
		mLocalTransformation(Move(localTransformation))
	{
		/// Nothing
	}
	...
	void TransformationNode::OnParentChanged(TransformationNode* previousParent)
	{
		// The whole subtree, not just this node: every world transformation below is
		// composed through this one, so all of them moved with it.
		SetDirty();
	}
