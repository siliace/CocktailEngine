#ifndef COCKTAILENGINE_CORE_ARRAY_HPP
#define COCKTAILENGINE_CORE_ARRAY_HPP

#include <CocktailEngine/Core/Memory/Asan.hpp>
#include <CocktailEngine/Core/Memory/ObjectMemoryUtils.hpp>
#include <CocktailEngine/Core/Memory/Allocator/AllocatorUtils.hpp>
#include <CocktailEngine/Core/Memory/Allocator/SizedHeapAllocator.hpp>
#include <CocktailEngine/Core/Utility/FunctionTraits.hpp>
...
namespace Ck
{
    ...
    public:
        ...
        E& Emplace(TArgs&&... args)
        {
            ...
            ObjectMemoryUtils::Construct(availableElements, Forward<TArgs>(args)...);
            ++mSize;
        ...
        {
            if (index == mSize)
                return Emplace(Forward<TArgs>(args)...);
            ...
            ObjectMemoryUtils::Construct(&element, Forward<TArgs>(args)...);
            ++mSize;
        ...
        void Prepend(const TOtherElement* elements, TOtherSizeType elementCount)
        {
            ...
            ObjectMemoryUtils::MoveRange(mSize, mData + elementCount, mData);
            ObjectMemoryUtils::CopyRange(elementCount, mData, elements);
        ...
        void Insert(SizeType index, std::initializer_list<TOtherElement> values)
        {
            ...
            for (SizeType i = 0; i < elementCount; ++i)
                ObjectMemoryUtils::Construct(&mData[mSize + i], Move(mData[index + i]));

            ObjectMemoryUtils::DestroyRange(elementCount, mData + index);

            SizeType i = 0;
            for (auto it = values.begin(); it != values.end(); ++it)
            {
                ObjectMemoryUtils::Construct(&mData[index + i], Move(*it));
                ++i;
            }
        ...
        void Insert(SizeType index, const TOtherElement* elements, TOtherSizeType elementCount)
        {
            ...
            for (SizeType i = 0; i < elementCount; ++i)
                ObjectMemoryUtils::Construct(&mData[index + i], Move(elements[i]));
        ...
        E PopFirst()
        {
            ...
            E& first = mData[0];
            E firstElement = Move(first);

            ObjectMemoryUtils::Destroy(&first);
            ObjectMemoryUtils::MoveRange(mSize - 1, mData, mData + 1);
            Asan::AnnotateContiguousContainer(mData, mCapacity, mSize, mSize - 1);
        ...
        E PopLast()
        {
            ...
            ElementType lastElement = Move(last);
        ...
        E RemoveAt(SizeType index)
        {
            ...
            {
                E removed = Move(At(index));
                for (SizeType i = index + 1; i < mSize; ++i)
                    At(i - 1) = Move(At(i));

                ObjectMemoryUtils::Destroy(&mData[mSize - 1]);
                Asan::AnnotateContiguousContainer(mData, mCapacity, mSize, mSize - 1);
        ...
        Array& FilterInPlace(TPredicate predicate)
        {
            ...
            for (SizeType readIndex = 0; readIndex < mSize; ++readIndex)
            {
                ...
                if (predicate(readElement))
                {
                    ...
                    {
                        ObjectMemoryUtils::Construct(&writeElement, Move(readElement));
            ...
            Asan::AnnotateContiguousContainer(mData, mCapacity, mSize, writeIndex);
            mSize = writeIndex;
        ...
        Array<typename FunctionTraits<TFunction>::Return> Transform(TFunction transformer) const
        {
            ...
            for (SizeType i = 0; i < mSize; ++i)
            {
                ...
                if constexpr (FunctionTraits<TFunction>::Arity == 0)
                {
                    ...
                    transformed.Add(Move(transformedElement));
                }
        ...
        T Reduce(T initialValue, TFunction function) const
        {
            ForEach([&](const ElementType& element) {
                initialValue = function(Move(initialValue), element);
            });
        ...
        Array& SpliceInPlace(SizeType first, SizeType count)
        {
            ...
            Asan::AnnotateContiguousContainer(mData, mCapacity, mSize, mSize - count);
            mSize -= count;
        ...
        void Clear()
        {
            if (mSize > 0)
            {
                ...
                Asan::AnnotateContiguousContainer(mData, mCapacity, mSize, mCapacity);
            }
    ...
    private:
        ...
        void ResizeAllocation(SizeType nextCapacity)
        {
            ...
            if (nextCapacity)
            {
               ...
               Asan::AnnotateContiguousContainer(data, nextCapacity, nextCapacity, mSize);
            }
...
#endif // COCKTAILENGINE_CORE_ARRAY_HPP
