#ifndef COCKTAILENGINE_CORE_MEMORY_OBJECTMEMORYUTILS_HPP
#define COCKTAILENGINE_CORE_MEMORY_OBJECTMEMORYUTILS_HPP
...
namespace Ck
{
	...
	public:
		...
		{
			new (element) T(Forward<TArgs>(args)...);
		}
		...
		static void ConstructRange(TSizeType size, T* elements, TArgs&&... args)
		{
			...
			for (TSizeType i = 0; i < size; ++i)
				new (&elements[i]) T(Forward<TArgs>(args)...);
		}
		...
		static void MoveRange(TSizeType size, T* destination, T* source)
		{
			...
			if constexpr (!std::is_trivially_copyable_v<T>)
			{
				if (destination < source)
				{
					...
					{
						T& element = source[i];
						new (&destination[i]) T(Move(element));
						element.~T();
					}
				}
				else
...
#endif // COCKTAILENGINE_CORE_MEMORY_OBJECTMEMORYUTILS_HPP
