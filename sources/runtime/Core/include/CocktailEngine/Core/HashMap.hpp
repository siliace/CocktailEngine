#ifndef COCKTAILENGINE_CORE_HASHMAP_HPP
#define COCKTAILENGINE_CORE_HASHMAP_HPP
...
namespace Ck
{
    ...
    struct MapEntry
    {
        ...
        MapEntry(TOtherKey&& key, TOtherValue&& value) :
            Key(Forward<TOtherKey>(key)),
            Value(Forward<TOtherValue>(value))
        {
        }
    ...
    namespace Detail
    {
        ...
        struct MapEntryPolicy
        {
            ...
            static void ReplaceValue(EntryType& entry, TOtherKey&& /*key*/, TOtherValue&& value)
            {
                ...
                ObjectMemoryUtils::Construct(&entry.Value, Forward<TOtherValue>(value));
            }
        };
    }
    ...
    public:
        ...
        {
            Base::PutEntry(key, Forward<TOtherKey>(key), Forward<TOtherValue>(value));
        }
        ...
        {
            return Base::PutEntryIfMissing(key, Forward<TOtherKey>(key), Forward<TOtherValue>(value));
        }
        ...
        ValueType& ComputeIfMissing(TOtherKey&& key, F&& function)
        {
            ...
            Node& node = bucket.Emplace(hashCode, Forward<TOtherKey>(key), Move(value));
            ++this->mSize;
        ...
        bool Replace(TOtherKey&& key, TOtherValue&& value)
        {
            return !Base::FindNode(static_cast<const KeyType&>(key))
                        .Then([&value](Node& node) {
                            node.Value = Forward<TOtherValue>(value);
                        })
                        .IsEmpty();
        }
        ...
        ValueType& GetOrAdd(TOtherKey&& key)
        {
            ...
            std::size_t hashCode;
            auto& bucket = Base::GetBucket(key, hashCode);
            Node& node = bucket.Emplace(hashCode, Forward<TOtherKey>(key), ValueType());
            ++this->mSize;
            if (this->mSize > this->mBucketCount * 3 / 4)
            {
                Base::Reserve(this->mSize);
...
#endif // COCKTAILENGINE_CORE_HASHMAP_HPP
