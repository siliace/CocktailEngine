...
namespace Ck
{
	...
	public:
		...
		void Trace(const LogCategory& category, LogLevel level, StringView message, StringView file, Uint64 line, Args&&... args)
		{
			...
			LogEntryPtr logEntry = mEntryPool.AllocateUnique();
			...
			mOnTraceEntry.Emit(logEntry.Get());
			mEntries.Add(Move(logEntry));
		}
		 ...
		 * \return 
		 */
		void GetEntries(Array<LogEntry*>& entries) const;
	...
	private:
		...
		using LogEntryPtr = ObjectPool<LogEntry>::Unique;

		HashMap<String, UniquePtr<LogChannel>> mChannels;
		Array<LogEntryPtr> mEntries;
