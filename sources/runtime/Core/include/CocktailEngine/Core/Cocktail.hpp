#ifndef COCKTAILENGINE_CORE_COCKTAIL_HPP
#define COCKTAILENGINE_CORE_COCKTAIL_HPP
...
#define COCKTAIL_PATCH_VERSION 0
	...
	#define COCKTAIL_COMPILER_MSC
		...
		#define CK_EXPORT_API __declspec(dllexport)
		#define CK_IMPORT_API __declspec(dllimport)
	#elif defined COCKTAIL_COMPILER_GCC
		#if COCKTAIL_COMPILER_GCC_VERSION_MAJOR >= 4
			#define CK_EXPORT_API __attribute__ ((__visibility__ ("default")))
			#define CK_IMPORT_API __attribute__ ((__visibility__ ("default")))
		#else
			#define CK_EXPORT_API
			#define CK_IMPORT_API
		#endif
	#else
		#define CK_EXPORT_API
		#define CK_IMPORT_API
...
namespace Ck
{
    ...
    template <typename T, typename TProduced>
    concept Producer = requires (T& v)
    {
        { v() } -> std::same_as<TProduced>;
    };

    template <typename T, typename TArg>
    concept Predicate = requires(T& v, TArg arg)
    {
        { v(arg) } -> std::same_as<bool>;
    };

    template <typename T, typename TArg>
    concept Consumer = requires(T& v, TArg arg)
    {
        { v(arg) } -> std::same_as<void>;
    };

    template <typename T, typename TArg1, typename TArg2>
    concept BiConsumer = requires(T& v, TArg1 arg1, TArg2 arg2)
    {
        { v(arg1, arg2) } -> std::same_as<void>;
    };

    template <typename T>
    constexpr T&& Forward(typename std::remove_reference_t<T>& value) noexcept
    {
    ...
    constexpr T&& Forward(typename std::remove_reference_t<T>&& value) noexcept
    {
        static_assert(!std::is_lvalue_reference_v<T>, "Forward called with an rvalue as an lvalue");
...
#endif // COCKTAILENGINE_CORE_COCKTAIL_HPP
