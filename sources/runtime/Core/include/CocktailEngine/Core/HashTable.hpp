#ifndef COCKTAILENGINE_CORE_HASHTABLE_HPP
#define COCKTAILENGINE_CORE_HASHTABLE_HPP

#include <CocktailEngine/Core/Array.hpp>
#include <CocktailEngine/Core/LinkedList.hpp>
...
namespace Ck::Detail
{
    ...
    protected:
        ...
        public:
            ...
            bool Put(std::size_t hashCode, const KeyType& key, TArgs&&... args)
            {
                ...
                while (head != nullptr)
                {
                    ...
                    {
                        TEntryPolicy::ReplaceValue(node, Forward<TArgs>(args)...);
                        return false;
                    }
                    head = head->GetNext();
                }

                LinkedListBase<Node, TAllocator>::EmplaceBack(hashCode, Forward<TArgs>(args)...);
                return true;
            }
            ...
            {
                return LinkedListBase<Node, TAllocator>::EmplaceBack(hashCode, Forward<TArgs>(args)...);
            }
        ...
        bool PutEntry(TLookupKey&& lookupKey, TArgs&&... args)
        {
            ...
            if (bucket.Put(hashCode, canonicalKey, Forward<TArgs>(args)...))
            {
                ++mSize;
                MaybeRehash();
                return true;
        ...
        bool PutEntryIfMissing(TLookupKey&& lookupKey, TArgs&&... args)
        {
            ...
            bucket.Emplace(hashCode, Forward<TArgs>(args)...);
            ++mSize;
            MaybeRehash();
            return true;
        }
        ...
        void Rehash(SizeType newBucketCount)
        {
            ...
            for (SizeType i = 0; i < mBucketCount; i++)
            {
                ...
                while (it.IsValid())
                {
                    ...
                    newBuckets[newIndex].Emplace(node.HashCode, Move(static_cast<EntryType&>(node)));
                    it = it.Next();
                }
            }

            mBuckets = Move(newBuckets);
...
#endif // COCKTAILENGINE_CORE_HASHTABLE_HPP
