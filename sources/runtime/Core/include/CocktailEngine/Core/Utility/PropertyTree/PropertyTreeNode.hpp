#ifndef COCKTAILENGINE_CORE_UTILITY_PROPERTYTREE_PROPERTYTREENODE_HPP
#define COCKTAILENGINE_CORE_UTILITY_PROPERTYTREE_PROPERTYTREENODE_HPP
    ...
    public:
        ...
        PropertyTreeNode<Data>* SetNextSibling(UniquePtr<PropertyTreeNode<Data>> node)
        {
            if (mNextSibling)
            {
                ...
                if (node)
					node->mNextSibling = Move(mNextSibling);
            }

            if (node)
				node->mPreviousSibling = this;

            mNextSibling = Move(node);
...
#endif // COCKTAILENGINE_CORE_UTILITY_PROPERTYTREE_PROPERTYTREENODE_HPP
