...
namespace Ck
{
	...
	Path Path::Parse(const TextChar* path, SizeType length, Format format)
    {
        ...
        return Path(Move(root), Move(elements), format);
    }
        ...
        mFormat(format)
    {
        if (string && length)
        {
            ...
            mRoot = Move(parsed.mRoot);
            mElements = Move(parsed.mElements);
            mJoined.SetDirty();
        }
    }
    ...
    Path Path::GetParent() const
    {
        ...
        return Path(mRoot, Move(elements), mFormat);
    }
