...
namespace Ck::Vulkan
{
    ...
    {
		QueueSubmitBatchPtr batch = mBatchPool.AllocateUnique(mRenderDevice, queueType, queueIndex);
	...
	void SubmitScheduler::ConnectFence(QueueSubmitBatch* batch, SharedPtr<Fence> fence)
	{
		...
		batch->Connect(batch->OnCompleted(), [this, batch = batch]() {
			...
			mPending.FindIndexIf([batch](const QueueSubmitBatchPtr& existingBatch) {
	...
	void SubmitScheduler::Submit()
	{
		...
		{
			QueueSubmitBatchPtr batch = mBatches.PopFirst();
