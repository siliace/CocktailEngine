...
namespace Ck::Vulkan
{
	...
	{
		mDescriptorSetLayout = createInfo.InDescriptorSetLayout;
		...
		VkDescriptorUpdateTemplateCreateInfoKHR vkCreateInfo{ VK_STRUCTURE_TYPE_DESCRIPTOR_UPDATE_TEMPLATE_CREATE_INFO_KHR, nullptr };
		{
			...
			if (mDescriptorSetLayout->SupportPushDescriptor())
			{
				...
				vkCreateInfo.pipelineBindPoint = createInfo.InPipelineLayout->GetBindPoint();
				vkCreateInfo.pipelineLayout = createInfo.InPipelineLayout->GetHandle();
