...
namespace Ck::Vulkan
{
	...
	{
		mChunks.RemoveIf([&](Ptr<DeviceMemoryChunk> chunk) {
			return chunk->IsFree();
		});
		...
		if (compact)
		{
			mChunks.ForEach([](Ptr<DeviceMemoryChunk> chunk) {
				chunk->Compact();
			});
		}
	}
    ...
    DeviceMemoryBlock* DeviceMemoryAllocator::AllocateBlock(Renderer::MemoryPriority priority, const VkMemoryRequirements& memoryRequirements, unsigned int memoryTypeIndex)
    {
	    ...
	    ObjectPoolUniquePtr<DeviceMemoryChunk> chunk = mChunkPool.Allocate(mRenderDevice, mBlockPool, priority, chunkSize, memoryTypeIndex);
    ...
    {
	    for (Ptr<DeviceMemoryChunk> chunk : mChunks)
	    {
