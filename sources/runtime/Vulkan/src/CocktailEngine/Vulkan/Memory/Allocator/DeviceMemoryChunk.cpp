...
namespace Ck::Vulkan
{
	...
	DeviceMemoryBlock* DeviceMemoryChunk::AllocateBlock(std::size_t alignment, std::size_t size)
	{
		return mBlocks.FindIndexIf([&](const auto& block) {
			...
			ObjectPoolUniquePtr<DeviceMemoryBlock> remainingBlock = allocatedBlock->Split(mBlockPool, alignment, size);
			if (remainingBlock)
				mBlocks.AddAt(index + 1, Move(remainingBlock));
	...
	{
		return mBlocks.AllOf([](const ObjectPoolUniquePtr<DeviceMemoryBlock>& block) {
			return block->IsFree();
		});
	}
	...
	void DeviceMemoryChunk::AllocateDeviceMemory(const AbstractTexture* texture, const Buffer* buffer)
	{
		...
		createInfo.TextureResource = texture;
		createInfo.BufferResource = buffer;
