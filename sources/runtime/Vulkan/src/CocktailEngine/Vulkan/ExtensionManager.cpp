...
namespace Ck::Vulkan
{
	...
	template <>
	struct FeatureRequirement<RenderDeviceFeature::PipelineCreationFeedback>
	{
		inline static unsigned int InstanceExtensionCount = 0;
		inline static const AnsiChar** InstanceExtensions = nullptr;
	...
	template <>
	struct FeatureRequirement<RenderDeviceFeature::MemoryBudget>
	{
		inline static unsigned int InstanceExtensionCount = 1;
		inline static const AnsiChar* InstanceExtensions[] = {
			VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME,
	...
	{
		mDisabledExtensions = App::GetEnvironmentVariable(CK_TEXT("COCKTAILENGINE_VULKAN_DISABLED_EXTENSIONS")).Map([&](const String& variable) {
...
#define CASE_FEATURE(__Feature)                                                      \
		...
		CASE_FEATURE(RenderDeviceFeature::PipelineCreationFeedback)
		CASE_FEATURE(RenderDeviceFeature::MemoryBudget)
		}
#undef CASE_FEATURE
