...
namespace Ck::Vulkan
{
	...
	void GraphicStateManager::SetShaderProgram(const ShaderProgram* shaderProgram)
	{
		...
		if (CheckedAssign(mShaderProgram, shaderProgram))
		{
			...
			for (Renderer::ShaderType shaderType : Enum<Renderer::ShaderType>::Values)
				mState.ShaderStages[shaderType].StageShader = mShaderStages[shaderType].Get();
		}
	}
	...
	{
		bool dirty = false;
		dirty |= CheckedAssign(mIndexBufferBinding.IndexBuffer, indexBuffer);
	...
	{
		bool dirty = false;
		dirty |= CheckedAssign(mVertexBufferBindings[binding].VertexBuffer, vertexBuffer);
	...
	SharedPtr<Pipeline> GraphicStateManager::CompilePipeline()
	{
		...
		createInfo.InRenderPass = mRenderPass;
	...
	IndexBufferBinding GraphicStateManager::CompileIndexBuffer()
	{
		...
		indexBufferBinding.BufferHandle = mIndexBufferBinding.IndexBuffer->GetHandle();
	...
	VertexBufferBindingRange GraphicStateManager::CompileVertexBuffers(unsigned int binding)
	{
		...
		{
			range.BufferHandles[bindingCount] = mVertexBufferBindings[binding].VertexBuffer->GetHandle();
