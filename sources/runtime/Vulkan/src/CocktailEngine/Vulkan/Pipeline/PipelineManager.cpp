...
namespace Ck::Vulkan
{
	namespace
	{
		...
		{
			Uint64 handleHash = 0;
			HashCombine(handleHash, static_cast<void*>(state.StageShader->GetHandle()));
		...
		PipelineStateHash HashGraphicState(const GraphicState& graphicState)
		{
...

			{
				...
				{
					if (graphicState.ShaderStages[shaderType].StageShader == nullptr)
						continue;
	...
	{
	    auto pipeline = mComputePipelines.ComputeIfMissing(HashComputeState(createInfo.State), [this, &createInfo](PipelineStateHash stateHash) {
            SharedPtr<ComputePipeline> computePipeline = mRenderDevice->CreateComputePipeline(mCache.Get(), createInfo);

            const PipelineCreationFeedbackEntry& feedback = computePipeline->GetCreationFeedback().Overall;
            if (!feedback.Valid)
            {
                CK_LOG(VulkanLogCategory, LogLevel::Info, CK_TEXT("Created ComputePipeline with hash %llu"), stateHash);
            }
            else if (feedback.ApplicationCacheHit)
            {
                CK_LOG(VulkanLogCategory, LogLevel::Info, CK_TEXT("Created ComputePipeline with hash %llu from the pipeline cache in %llu us"), stateHash, feedback.Duration / 1000);
            }
            else
            {
                CK_LOG(VulkanLogCategory, LogLevel::Info, CK_TEXT("Created ComputePipeline with hash %llu, compiled in %llu us"), stateHash, feedback.Duration / 1000);
            }
	...
	{
	    auto pipeline = mGraphicPipelines.ComputeIfMissing(HashGraphicState(createInfo.State), [this, &createInfo](PipelineStateHash stateHash) {
            SharedPtr<GraphicPipeline> graphicPipeline = mRenderDevice->CreateGraphicPipeline(mCache.Get(), createInfo);

	        const PipelineCreationFeedbackEntry& feedback = graphicPipeline->GetCreationFeedback().Overall;
            if (!feedback.Valid)
            {
            ...
            {
                CK_LOG(VulkanLogCategory, LogLevel::Info, CK_TEXT("Created GraphicPipeline with hash %llu from the pipeline cache in %llu us"), stateHash, feedback.Duration / 1000);
            }
            else
            {
                CK_LOG(VulkanLogCategory, LogLevel::Info, CK_TEXT("Created GraphicPipeline with hash %llu, compiled in %llu us"), stateHash, feedback.Duration / 1000);
            }
