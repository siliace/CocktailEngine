...
namespace Ck::Vulkan
{
		...
		mHandle(VK_NULL_HANDLE)
	{
		...
		VkPipelineShaderStageCreateInfo vkPipelineShaderStages[Enum<Renderer::ShaderType>::ValueCount];
		{
			...
			{
				const ShaderStageState& shaderStageState = createInfo.State.ShaderStages[type];

				const Shader* shaderStage = shaderStageState.StageShader;
				if (!shaderStage)
					continue;
		...
		{
			const VertexInputState& vertexInputState = createInfo.State.VertexInput;
		...
		{
			const InputAssemblyState& inputAssemblyState = createInfo.State.InputAssembly;
		...
		{
			const ViewportState& viewportState = createInfo.State.Viewport;
		...
		{
			const RasterizationState& rasterizationState = createInfo.State.Rasterization;
		...
		{
			const MultisampleState& multisampleState = createInfo.State.Multisample;
		...
		{
			const DepthStencilState& depthStencilState = createInfo.State.DepthStencil;
		...
		{
			const ColorBlendState& colorBlendState = createInfo.State.ColorBlend;
		...
		Array<VkDynamicState, LinearAllocator<32>> vkDynamicStates = { VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR };
		...
		// into outlives the vkCreateGraphicsPipelines call below.
		PipelineCreationFeedbackCollector creationFeedbackCollector(mRenderDevice, shaderStageCount);
		...
		VkGraphicsPipelineCreateInfo vkCreateInfo{ VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO, nullptr };
		{
			...
			vkCreateInfo.renderPass = createInfo.InRenderPass->GetHandle(Renderer::RenderPassMode::Initial);
	...
	SharedPtr<PipelineLayout> GraphicPipeline::GetLayout() const
	{
		return mLayout;
	}
