...
# out of VirtualAlloc and mmap. Those regions are not registered sanitizer heap
# allocations, but BinnedAllocator now drives their shadow explicitly: only exact
# live payloads are exposed, allocation redzones and freed slots are poisoned, and
# remote frees are poisoned before publication. Engine-memory redzone overruns and
# use-after-free accesses are therefore detected while the backing region remains
# mapped.
#
# Manual shadow management does not turn these regions into registered sanitizer heap
# allocations. They still get no sanitizer heap quarantine, no heap allocation/free
# stacks, and no automatic leak tracking. The build additionally retains checks that
# do not depend on the allocator, including stack and global checks, plus whatever an
# uninstrumented dependency allocates through an intercepted system allocator.
#
# _ck_sanitizer_uniform_definitions continues to disable standard-library container
# annotations uniformly across each MSVC binary; BinnedAllocator remains the owner of
# shadow state for engine allocations.
# -----------------------------------------------------------------------------
...
#
# This build turns the annotations off everywhere to settle that binary-contract
# requirement. The policy stays uniform even though BinnedAllocator now resets and
# rebuilds the shadow for every engine allocation itself: allocation-level redzones
# and free poisoning are owned by the allocator, while standard-library size/capacity
# annotations remain intentionally disabled.
#
...
function(_ck_configure_sanitizer target)
    ...
    target_compile_definitions(${target} PRIVATE CK_ASAN)
    if(link_options)
...
function(_ck_configure_shared_module target namespace name)
    ...
    set_target_properties(${target} PROPERTIES DEFINE_SYMBOL "${build_macro}")
endfunction()
