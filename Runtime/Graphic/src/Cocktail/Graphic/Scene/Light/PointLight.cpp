...
namespace Ck
{
    PointLight* PointLight::Create(std::shared_ptr<Scene> scene, String name, float range, LinearColor color, Vector3<float> position, float intensity)
    {
        ...
        UniquePtr<PointLight> pointLight = MakeUnique<PointLight>(std::move(transformationNode), std::move(name), range, color, intensity);
    ...
    PointLight::PointLight(std::shared_ptr<TransformationNode> transformationNode, String name,  float range, LinearColor color, float intensity) :
        PositionalLight(std::move(transformationNode), std::move(name), range, color, intensity)
    {
        /// Nothing
    }
