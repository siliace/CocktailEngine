...
namespace Ck
{
	...
	{
		mSceneGraph = SceneGraph::New(mTransformationGraph->GetRoot());
	}
	...
	Ref<SceneNode> Scene::CreateSceneNode()
	{
		...
		Ref<SceneNode> sceneNode = mSceneGraph->CreateNode(std::move(transformationNode));
	...
	Ref<TransformationNode> Scene::CreateTransformationNode(const Transformation& transformation) const
	{
