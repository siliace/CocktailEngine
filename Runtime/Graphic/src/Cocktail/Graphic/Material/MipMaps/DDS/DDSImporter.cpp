...
namespace Ck
{
	...
	Ref<MipMaps> DDSImporter::LoadFromPath(const std::filesystem::path& path, const MipMapsImportParameters& parameters) const
	{
		return LoadFromStream(
			FileInputStream::New(Storage::OpenFile(path, FileOpenFlagBits::Read)), parameters
		);
	}

	Ref<MipMaps> DDSImporter::LoadFromStream(const Ref<InputStream>& inputStream, const MipMapsImportParameters& parameters) const
	{
		...
		unsigned int mipMapCount = GetTextureLevelCount(header);

		std::size_t topLevelSize = pixelFormat.ComputeAllocationSize(baseSize);
		std::unique_ptr<unsigned char[]> pixels = std::make_unique<unsigned char[]>(topLevelSize);
	...
	Ref<MipMaps> DDSImporter::LoadFromMemory(const void* buffer, std::size_t length, const MipMapsImportParameters& parameters) const
	{
		return LoadFromStream(
