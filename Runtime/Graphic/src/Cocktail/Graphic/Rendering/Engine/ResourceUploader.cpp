...
namespace Ck
{
	void ResourceUploader::RequestBufferUpload(Renderer::Buffer* buffer, std::size_t offset, std::size_t length, const void* data)
	{
	...
	void ResourceUploader::RequestTextureUpload(Renderer::Texture* texture, unsigned int arrayLayer, unsigned int mipMapLevel, const void* data)
	{
	...
	{
		return !mBufferUploadRequests.empty();
	}

	void ResourceUploader::Flush(Renderer::CommandList& commandList)
	{
		for (const auto& bufferUploadRequest : mBufferUploadRequests)
			commandList.UploadBuffer(bufferUploadRequest.Buffer, bufferUploadRequest.UploadInfo.size(), bufferUploadRequest.UploadInfo.data());
		...
		{
			{
				...
				for (std::size_t i = 0; i < textureUploadRequest.UploadInfo.size(); i++)
				{
					...
					barriers[i] = Renderer::GpuBarrier::Of(
						textureUploadRequest.Texture, Renderer::ResourceState::Undefined, Renderer::ResourceState::CopyDestination,
						subResource
					);
				}
			...
			commandList.UploadTexture(textureUploadRequest.Texture, Renderer::ResourceState::CopyDestination, textureUploadRequest.UploadInfo.size(), textureUploadRequest.UploadInfo.data());

			{
				...
				for (std::size_t i = 0; i < textureUploadRequest.UploadInfo.size(); i++)
				{
					...
					barriers[i] = Renderer::GpuBarrier::Of(
						textureUploadRequest.Texture, Renderer::ResourceState::CopyDestination, Renderer::ResourceState::GraphicShaderResource,
						subResource
					);
				}
				commandList.Barrier(textureUploadRequest.UploadInfo.size(), barriers);
