...
namespace Ck
{
	VertexBuffer::VertexBuffer(Ref<GraphicEngine> graphicEngine, Ref<VertexArray> vertices, std::string_view name) :
		Extends<VertexBuffer, BufferResource>(std::move(graphicEngine), Renderer::BufferUsageFlagBits::Vertex, vertices->GetSize(), name),
	...
	void VertexBuffer::Upload(std::size_t firstVertex, std::size_t vertexCount)
	{
		...
		GetGraphicEngine()->UploadBuffer(Self(), firstVertexOffset, length, (const Uint8*)mVertices->GetPointer() + firstVertexOffset);
	}
