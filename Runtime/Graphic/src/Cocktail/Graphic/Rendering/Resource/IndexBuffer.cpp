...
namespace Ck
{
	IndexBuffer::IndexBuffer(Ref<GraphicEngine> graphicEngine, Ref<IndexArray> indices, std::string_view name) :
		Extends<Ck::IndexBuffer, Ck::BufferResource>(std::move(graphicEngine), Renderer::BufferUsageFlagBits::Index, indices->GetSize(), name),
	...
	void IndexBuffer::Upload(std::size_t firstIndex, std::size_t indexCount)
	{
		...
		GetGraphicEngine()->UploadBuffer(Self(), firstVertexOffset, length, (const Uint8*)mIndices->GetPointer() + firstVertexOffset);
	}
