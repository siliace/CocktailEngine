...
namespace Ck
{
	...
	void RecordDrawContext::BindMaterialProgram(Renderer::CommandList& commandList, const MaterialProgram& materialProgram)
	{
		...
		for (const auto& [key, area] : mPersistentUniformBuffers)
		{
	...
	void RecordDrawContext::BindUniformData(Renderer::CommandList& commandList, std::string_view name, unsigned int arrayIndex, std::size_t size, const void* data) const
	{
		...
		Renderer::BufferArea area = mFrameContext->GetBufferAllocator(Renderer::BufferUsageFlagBits::Uniform, Renderer::MemoryType::Unified)->PushData(allocationSize, data);
	...
	void RecordDrawContext::BindPersistentUniformData(Renderer::CommandList& commandList, std::string_view name, unsigned int arrayIndex, std::size_t size, const void* data)
	{
		std::size_t allocationSize = size;
		Renderer::BufferArea area = mFrameContext->GetBufferAllocator(Renderer::BufferUsageFlagBits::Uniform, Renderer::MemoryType::Unified)->PushData(allocationSize, data);

		std::string n(name.data(), name.length());
		mPersistentUniformBuffers[CompositeKey<std::string, unsigned int>(n, arrayIndex)] = area;
	...
	void RecordDrawContext::SetVertexInputAttributes(Renderer::CommandList& commandList, unsigned int binding, const VertexLayout& vertexLayout) const
	{
		...
		for (std::size_t i = 0; i < attributes.size(); i++)
		{
			...
			{
				Log::Error("Missing VertexAttribute {} in MaterialProgram {}", Enum<VertexAttributeSemantic>::ToString(attribute.GetSemantic()), mCurrentMaterialProgram->GetName());
				continue;
			}
