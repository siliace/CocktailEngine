...
namespace Ck
{
	...
	public:
		 ...
		 * \param data
		 */
		void BindPersistentUniformData(Renderer::CommandList& commandList, std::string_view name, unsigned int arrayIndex, std::size_t size, const void* data);
	...
	private:
		...
		std::unordered_map<CompositeKey<std::string, unsigned int>, Renderer::BufferArea> mPersistentUniformBuffers;
	};
}
