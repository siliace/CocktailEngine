...
namespace Ck
{
	...
	public:
		 ...
		 */
		template <typename T>
		void Set(VertexAttributeSemantic semantic, const RgbColor<T>& color)
		{
			T value[] = { color.Red, color.Green, color.Blue };
			Set<T>(semantic, value, 3);
		}
		 ...
		 */
		template <typename T>
		void Set(VertexAttributeSemantic semantic, const RgbaColor<T>& color)
		{
			T value[] = { color.Red, color.Green, color.Blue, color.Alpha };
			Set<T>(semantic, value, 4);
		}
