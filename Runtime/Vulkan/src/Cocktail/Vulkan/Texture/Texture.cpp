...
namespace Ck::Vulkan
{
	Texture::Texture(const Ref<RenderDevice>& renderDevice, const Renderer::TextureCreateInfo& createInfo, const VkAllocationCallbacks* allocationCallbacks) :
		Super(renderDevice),
		...
		mAllocationCallbacks(allocationCallbacks)
	{
		...
		VkImageCreateInfo vkCreateInfo{ VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO, nullptr };
		{
			...
			vkCreateInfo.usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
		...
		mDeviceMemoryBlock = mRenderDevice->Invoke([&](const Ref<DeviceMemoryAllocator>& deviceMemoryAllocator) {
