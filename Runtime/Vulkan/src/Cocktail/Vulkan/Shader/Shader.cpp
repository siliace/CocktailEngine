...
namespace Ck::Vulkan
{
	...
	Shader::Shader(const Ref<RenderDevice>& renderDevice, const ValidationCache* validationCache, const Renderer::ShaderCreateInfo& createInfo, const VkAllocationCallbacks* allocationCallbacks) :
			...
			if (reflectDescriptorSetCount)
			{
				...
				for (unsigned int i = 0; i < mDescriptorSetInfo.size(); i++)
				{
					...
					for (unsigned int j = 0; j < reflectDescriptorSet->binding_count; j++)
					{
						...
						descriptorSetInfo.Bindings[j].Type = ToCkType(reflectBinding->descriptor_type);

						if (descriptorSetInfo.Bindings[j].Type == Renderer::DescriptorType::UniformBuffer)
						{
	...
	Ref<Renderer::RenderDevice> Shader::GetRenderDevice() const
	{
