...
namespace Ck::Vulkan
{
	...
	Shader::Shader(const Ref<RenderDevice>& renderDevice, const Renderer::ShaderCreateInfo& createInfo, const VkAllocationCallbacks* allocationCallbacks) :
		...
		// Reflect push constant
		{
			...
			if (reflectPushConstantBlockCount)
			{
				...
				for (unsigned int i = 0; i < mPushConstantBlockInfo.size(); i++)
				{
					...
					pushConstantBlockInfo.BaseOffset = 0;
