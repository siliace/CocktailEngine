...
namespace Ck::Vulkan
{
	...
	void ShaderProgram::CreatePipelineLayout()
	{
		...
		if (mRenderDevice->IsFeatureSupported(RenderDeviceFeature::PushDescriptors))
		{
			unsigned int notEmptyLayoutCount = 0;
			DescriptorSetLayoutCreateInfo* firstNotEmptyLayout = nullptr;
			for (auto& [descriptorSetIndex, layoutCreateInfo] : descriptorSetLayoutsCreateInfo)
			{
				if (descriptorSetLayoutsBindings[descriptorSetIndex].empty())
					continue;

				if (!firstNotEmptyLayout)
					firstNotEmptyLayout = &layoutCreateInfo;

				++notEmptyLayoutCount;
			}

			if (notEmptyLayoutCount == 1)
			{
				...
				firstNotEmptyLayout->SupportPushDescriptor = firstNotEmptyLayout->BindingCount <= pushDescriptorProperties.maxPushDescriptors;
			}
		}
