...
namespace Ck::Vulkan
{
	...
	void RenderDevice::InitializeMemoryAllocator()
	{
		mDeviceMemoryAllocator = DeviceMemoryAllocator::New(Self());
	}

	void RenderDevice::InitializeDebugMessenger()
	{
		DebugMessengerCreateInfo messengerCreateInfo;
		mDebugMessenger = DebugMessenger::New(Self(), messengerCreateInfo, nullptr);
	}

	void RenderDevice::InitializePipelineCache()
	{
		PipelineCacheCreateInfo cacheCreateInfo;
		mPipelineCache = PipelineCache::New(Self(), cacheCreateInfo, nullptr);
	}

	DeviceMemoryBlock* RenderDevice::AllocateBufferMemory(Buffer* buffer)
	{
		return mDeviceMemoryAllocator->Allocate(buffer);
	}

	DeviceMemoryBlock* RenderDevice::AllocateTextureMemory(AbstractTexture* texture)
	{
		return mDeviceMemoryAllocator->Allocate(texture);
	}

	void RenderDevice::GarbageCollect(bool compact)
	{
		mDeviceMemoryAllocator->GarbageCollect(compact);
	}
	...
	{
		return mPipelineCache->CreateComputePipeline(createInfo);
	}
	...
	{
		return mFramebufferPool.Allocate(Self(), renderPass, createInfo, nullptr);
	}
	
	Ref<GraphicPipeline> RenderDevice::CreateGraphicPipeline(const GraphicPipelineCreateInfo& createInfo)
	{
		return mPipelineCache->CreateGraphicPipeline(createInfo);
	}
	...
	{
		return mSamplerCache.GetOrCreate(createInfo, [&]() {
			return mSamplerPool.Allocate(Self(), createInfo, nullptr);
		});
	}
	...
	{
		return mShaderPool.Allocate(Self(), createInfo, nullptr);
	}
	...
	Signal<std::string_view>& RenderDevice::OnDebugMessage()
	{
	...
	void RenderDevice::CreateInstance(std::string_view applicationName, const VersionDescriptor& applicationVersion, ApiVersion apiVersion, bool enableValidationLayer)
	{
		...
		const std::vector<const char*>& enabledExtensionNames = mExtensionManager.GetInstanceExtensions();
	...
	void RenderDevice::ChoosePhysicalDevice()
	{
		auto rankingFunction = [](VkPhysicalDevice physicalDevice) {
			...
			{
				unsigned int heapScore = memoryProperties.memoryHeaps[i].size / (1024 * 1024);
	...
	void RenderDevice::CreateDevice()
	{
		...
		if (hasPhysicalDeviceFeatures2)
		{
			...
			Chain(timelineSemaphoreFeatures, synchronization2Features);
		...
		const std::vector<const char*>& enabledExtensionNames = mExtensionManager.GetDeviceExtensions();
