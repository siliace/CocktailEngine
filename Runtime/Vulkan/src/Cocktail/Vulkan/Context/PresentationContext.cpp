...
namespace Ck::Vulkan
{
	...
	PresentationContext::PresentationContext(const Ref<RenderDevice>& renderDevice, const PresentationContextCreateInfo& createInfo, const VkAllocationCallbacks* allocationCallbacks) :
		mRenderDevice(renderDevice),
		mSurface(createInfo.Surface),
		mBufferCount(createInfo.BufferCount),
		mAllocationCallbacks(allocationCallbacks)
	{
		...
		mSurfaceFormat = ChooseSurfaceFormat(physicalDevice, mSurface, createInfo.ColorDepth, createInfo.AlphaDepth, createInfo.ColorSpace);
		mSurfaceColorSpace = createInfo.ColorSpace;
	}
	...
	Ref<Swapchain> PresentationContext::CreateSwapchain(const Extent2D<unsigned int>& size, VkPresentModeKHR presentMode, Swapchain* old) const
	{
		...
		createInfo.MinImageCount = std::clamp(mBufferCount, surfaceCapabilities.minImageCount, surfaceCapabilities.maxImageCount);
	...
	unsigned PresentationContext::GetBufferCount() const
	{
		return mBufferCount;
	}
}
