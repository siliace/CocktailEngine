...
namespace Ck::Vulkan
{
	...
	public:
		 ...
		 */
		RenderContext(const Ref<RenderDevice>& renderDevice, const Renderer::RenderContextCreateInfo& createInfo, const VkAllocationCallbacks* allocationCallbacks);
		 ...
		 * \param fence 
		 */
		void ExecuteCommandLists(Renderer::CommandQueueType commandQueue, unsigned int commandListCount, Ref<Renderer::CommandList>* commandLists, const Ref<Renderer::Fence>& fence) override;
		 ...
		 * \return 
		 */
		Signal<Renderer::FrameContext*>& OnBeforeRedraw() override;

		/**
		 * \brief 
		 * \return 
		 */
		Signal<Renderer::FrameContext*, const Renderer::Framebuffer*>& OnRedraw() override;

	private:

		Ref<RenderDevice> mRenderDevice;
		Ref<RenderSurface> mRenderSurface;
