...
namespace Ck::Vulkan
{
	...
	public:
		 ...
		 * \return 
		 */
		unsigned int AcquireNextFramebuffer(Uint64 timeout, const Ref<Semaphore>& semaphore, const Ref<Fence>& fence);
		 ...
		 * \return
		 */
		Signal<>& OnClose() override;

		/**
		 * \brief 
		 * \return 
		 */
		Signal<Extent2D<unsigned int>>& OnResized() override;

		/**
		 * \brief 
		 * \return 
		 */
		Signal<>& OnRedraw() override;
