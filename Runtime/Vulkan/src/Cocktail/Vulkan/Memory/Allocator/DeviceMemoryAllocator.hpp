...
namespace Ck::Vulkan
{
	...
	public:
		 ...
		 * \param renderDevice 
		 */
		DeviceMemoryAllocator(Ref<RenderDevice> renderDevice, std::size_t chunkSize);
	...
	private:
		...
		DeviceMemoryBlock* AllocateBlock(const T* resource, const VkMemoryRequirements& memoryRequirements, unsigned int memoryTypeIndex)
		{
			...
			else
			{
				...
				if (!block)
				{
					Ref<DeviceMemoryChunk> chunk = mChunkPool.Allocate(mRenderDevice, mBlockPool, std::max(memoryRequirements.size, mChunkSize), memoryTypeIndex);
					mChunks.push_back(chunk);
