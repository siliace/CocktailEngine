...
namespace Ck::Vulkan
{
	...
	DeviceMemoryBlock* DeviceMemoryAllocator::Allocate(AbstractTexture* texture)
	{
		bool dedicated;
	...
	DeviceMemoryBlock* DeviceMemoryAllocator::Allocate(Buffer* buffer)
	{
		bool dedicated;
		VkMemoryRequirements memoryRequirements;
	...
	void DeviceMemoryAllocator::GetBufferMemoryRequirements(const Buffer* buffer, VkMemoryRequirements& requirements, bool& dedicated) const
	{
		...
		const bool supportedPriorityAllocation = mRenderDevice->IsFeatureSupported(RenderDeviceFeature::MemoryPriority);

		if (supportedDedicatedAllocation || supportedPriorityAllocation)
		{
