...
namespace Ck::Vulkan
{
		...
		mAllocationCallbacks(allocationCallbacks)
	{
		...
		for (Renderer::RenderPassMode renderPassMode : Enum<Renderer::RenderPassMode>::Values)
		{
			...
			if (mRenderDevice->IsFeatureSupported(RenderDeviceFeature::RenderPass2))
			{
				...
				{
					resolveDepthStencil.depthResolveMode = depthStencilFormat.IsDepth() || depthStencilFormat.IsDepthStencil() ? VK_RESOLVE_MODE_AVERAGE_BIT_KHR : VK_RESOLVE_MODE_NONE_KHR;
					resolveDepthStencil.stencilResolveMode = depthStencilFormat.IsStencil() || depthStencilFormat.IsDepthStencil() ? VK_RESOLVE_MODE_MIN_BIT : VK_RESOLVE_MODE_NONE_KHR;
	...
	bool RenderPass::SupportDepthStencilResolve() const
	{
		return mRenderDevice->IsFeatureSupported(RenderDeviceFeature::RenderPass2) && mRenderDevice->IsFeatureSupported(RenderDeviceFeature::RenderPassDepthStencilResolve);
	}
