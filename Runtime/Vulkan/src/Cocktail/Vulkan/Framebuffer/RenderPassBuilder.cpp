...
namespace Ck::Vulkan
{
	...
	VkAttachmentDescription RenderPassBuilder::CreateAttachmentDescription(const PixelFormat& format, Renderer::RasterizationSamples samples, VkAttachmentLoadOp loadOp, VkAttachmentStoreOp storeOp, bool presentable)
	{
		...
		attachmentDescription.storeOp = isMultisample ? VK_ATTACHMENT_STORE_OP_DONT_CARE : storeOp;
		...
		case VK_ATTACHMENT_LOAD_OP_LOAD:
			attachmentDescription.initialLayout = format.IsColor() ? VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL : VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
			break;
		...
		if (format.IsColor())
		{
			attachmentDescription.finalLayout = presentable ? VK_IMAGE_LAYOUT_PRESENT_SRC_KHR : VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
		}
		else
		{
