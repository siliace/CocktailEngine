...
namespace Ck::Vulkan
{
	 ...
	 * \brief 
	 */
	class RenderDevice : public Inherit<RenderDevice, Object, Renderer::RenderDevice>
	{
	public:
		 ...
		 * \return 
		 */
		DeviceMemoryBlock* AllocateBufferMemory(Buffer* buffer);
		 ...
		 * \return  
		 */
		DeviceMemoryBlock* AllocateTextureMemory(AbstractTexture* texture);
		
		/**
		 * \brief 
		 * \param compact 
		 */
		void GarbageCollect(bool compact = true);
		 ...
		 * \return 
		 */
		Signal<std::string_view>& OnDebugMessage() override;
	...
	private:
		...
		VkDevice mHandle;
		Ref<DebugMessenger> mDebugMessenger;
		Ref<PipelineCache> mPipelineCache;
		Ref<DeviceMemoryAllocator> mDeviceMemoryAllocator;
		...
		CachePool<Renderer::SamplerCreateInfo, Ref<Sampler>> mSamplerCache;
		...
		Signal<std::string_view> mOnDebugMessage;
	};
}
