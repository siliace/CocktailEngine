#include <Cocktail/Renderer/Command/CommandQueueType.hpp>

#include <Cocktail/Vulkan/RenderDevice.hpp>
#include <Cocktail/Vulkan/Queue/SubmitScheduler.hpp>

namespace Ck::Vulkan
{
	SubmitScheduler::SubmitScheduler(Ref<RenderDevice> renderDevice) :
		mRenderDevice(std::move(renderDevice))
	{
		/// Nothing
	}

	Ref<QueueSubmitBatch> SubmitScheduler::ScheduleBatch(Renderer::CommandQueueType queueType, unsigned int queueIndex)
	{
		Ref<QueueSubmitBatch> batch = mBatchPool.Allocate(mRenderDevice, queueType, queueIndex);
		mBatches.push(batch);

		return batch;
	}

	void SubmitScheduler::ConnectFence(const Ref<QueueSubmitBatch>& batch, const Ref<Fence>& fence)
	{
		batch->AssignFence(fence);
		batch->Connect(batch->OnCompleted(), [&](QueueSubmitBatch* completedBatch) {
			auto itBatch = std::find_if(mPending.begin(), mPending.end(), [&](const Ref<QueueSubmitBatch>& pendingBatch) {
				return pendingBatch.Get() == completedBatch;
			});

			if (itBatch != mPending.end())
			{
				std::for_each(mPending.begin(), itBatch, [](const Ref<QueueSubmitBatch>& pendingBatch) {
					pendingBatch->MarkCompleted();
				});

				mTerminated.insert(mTerminated.end(), mPending.begin(), std::next(itBatch));

				mPending.erase(mPending.begin(), std::next(itBatch));
			}
		});
	}

	void SubmitScheduler::Flush()
	{
		mTerminated.clear();

		while (!mBatches.empty())
		{
			Ref<QueueSubmitBatch> batch = std::move(mBatches.front());
			mBatches.pop();

			batch->Submit();
			mPending.emplace_back(std::move(batch));
		}
	}

	void SubmitScheduler::Synchronize()
	{
		vkDeviceWaitIdle(mRenderDevice->GetHandle());

		mPending.clear();
		mTerminated.clear();
	}
}
