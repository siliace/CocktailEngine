...
namespace Ck::Vulkan
{
	...
	public:
		...
		CommandList(Ref<RenderDevice> renderDevice, Ref<CommandListPool> allocator, const Ref<DescriptorSetAllocator>& descriptorSetAllocator, const Renderer::CommandListCreateInfo& createInfo, const VkAllocationCallbacks* allocationCallbacks);
		...
		void GenerateMipMaps(const Renderer::Texture* texture, const Renderer::TextureSubResource& subResource) override;
		...
		void BindUniformBuffer(Renderer::UniformSlot* slot, unsigned int arrayIndex, const Renderer::Buffer* uniformBuffer, std::size_t offset, std::size_t range) override;
		...
		void SetBlendingConstants(const RgbaColor<float>& blendingConstants) override;
	...
	private:
		...
		Ref<ComputeStateManager> GetComputeStateManager() const;
		Ref<GraphicStateManager> GetGraphicStateManager() const;
		...
		EnumMap<Renderer::ShaderProgramType, Ref<StateManager>> mStateManagers;
	};
}
