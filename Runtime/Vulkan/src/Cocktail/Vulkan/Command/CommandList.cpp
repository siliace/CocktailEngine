...
namespace Ck::Vulkan
{
	...
	CommandList::CommandList(Ref<RenderDevice> renderDevice, Ref<CommandListPool> allocator, const Ref<DescriptorSetAllocator>& descriptorSetAllocator, const Renderer::CommandListCreateInfo& createInfo) :
		...
		mDepthStencilResolver(mRenderDevice)
	{
		...
		{
			mStateManagers[Renderer::ShaderProgramType::Compute] = ComputeStateManager::New(mRenderDevice, descriptorSetAllocator);

			if (mUsage == Renderer::CommandListUsage::Graphic)
				mStateManagers[Renderer::ShaderProgramType::Graphic] = GraphicStateManager::New(mRenderDevice, descriptorSetAllocator, mDynamicState);
		}
	...
	void CommandList::GenerateMipMaps(const Renderer::Texture* texture, const Renderer::TextureSubResource& subResource)
	{
		...
		VkRenderPassBeginInfo beginInfo{ VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO, nullptr };
		{
			...
			if (begin.Mode == Renderer::RenderPassMode::Clear)
			{
				for (unsigned int i = 0; i < mCurrentFramebuffer->GetColorAttachmentCount(); i++)
				{
					...
					clearValue.color.float32[0] = begin.ColorClearValue[i].Red / 255.f;
					clearValue.color.float32[1] = begin.ColorClearValue[i].Green / 255.f;
					clearValue.color.float32[2] = begin.ColorClearValue[i].Blue / 255.f;
					clearValue.color.float32[3] = begin.ColorClearValue[i].Alpha / 255.f;
	...
	void CommandList::EndRenderPass()
	{
		...
		if (!mCurrentFramebuffer->GetRenderPass()->SupportDepthStencilResolve())
		{
			Ref<Renderer::TextureView> depthStencilAttachment = mCurrentFramebuffer->GetDepthStencilAttachment();
			if (depthStencilAttachment)
				mDepthStencilResolver.Resolve(*this, mCurrentFramebuffer->GetDepthStencilMultisampleAttachment(), depthStencilAttachment);
		}
	...
	{
		mStateManagers[shaderProgram->GetType()]->SetShaderProgram(ShaderProgram::Cast(shaderProgram));
	}
	...
	{
		if (!(vertexBuffer->GetUsage() & Renderer::BufferUsageFlagBits::Vertex))
			throw std::invalid_argument("Buffer was not create with the Index usage");

		const Ref<GraphicStateManager> graphicStateManager = GetGraphicStateManager();
		graphicStateManager->BindVertexBuffer(binding, Buffer::Cast(vertexBuffer), offset);
	...
	{
		if (!(indexBuffer->GetUsage() & Renderer::BufferUsageFlagBits::Index))
			throw std::invalid_argument("Buffer was not create with the Index usage");

		GetGraphicStateManager()->BindIndexBuffer(Buffer::Cast(indexBuffer), offset, indexType);
	}
	...
	{
		assert(!slot->IsArray() || arrayIndex < slot->GetArrayLength());
		assert(slot->GetDescriptorType() == Renderer::DescriptorType::Texture || slot->GetDescriptorType() == Renderer::DescriptorType::StorageTexture);
		...
		if (auto descriptorSetSlot = DescriptorSetSlot::TryCast(slot))
		{
			...
			if (slot->GetDescriptorType() == Renderer::DescriptorType::Texture)
			{
				mStateManagers[descriptorSetSlot->GetProgramType()]->BindTexture(set, binding, arrayIndex, TextureView::Cast(textureView));
			}
			else if (slot->GetDescriptorType() == Renderer::DescriptorType::StorageTexture)
			{
				mStateManagers[descriptorSetSlot->GetProgramType()]->BindStorageTexture(set, binding, arrayIndex, TextureView::Cast(textureView));
			}
		}
	}

	void CommandList::BindUniformBuffer(Renderer::UniformSlot* slot, unsigned int arrayIndex, const Renderer::Buffer* uniformBuffer, std::size_t offset, std::size_t range)
	{
		assert(!slot->IsArray() || arrayIndex < slot->GetArrayLength());
		assert(uniformBuffer->GetUsage() & Renderer::BufferUsageFlagBits::Uniform);
		assert(slot->GetDescriptorType() == Renderer::DescriptorType::UniformBuffer);

		if (auto descriptorSetSlot = DescriptorSetSlot::TryCast(slot))
		{
			unsigned int set = descriptorSetSlot->GetSet();
			unsigned int binding = descriptorSetSlot->GetBinding();
			mStateManagers[descriptorSetSlot->GetProgramType()]->BindUniformBuffer(set, binding, arrayIndex, Buffer::Cast(uniformBuffer), offset, range);
		}
	}

	void CommandList::UpdateBuffer(const Renderer::Buffer* buffer, std::size_t offset, std::size_t length, const void* data)
	{
		assert(mState == Renderer::CommandListState::Recording);

		if (length >= 65536)
		{
			Renderer::BufferUploadInfo uploadInfo;
			uploadInfo.Offset = offset;
			uploadInfo.Length = length;
			uploadInfo.Data = data;

			UploadBuffer(buffer, 1, &uploadInfo);
		}
		else
		{
			vkCmdUpdateBuffer(mHandle, Buffer::Cast(buffer)->GetHandle(), offset, length, data);
		}
	}
	...
	void CommandList::UpdatePipelineConstant(Renderer::UniformSlot* slot, unsigned int offset, unsigned int length, const void* data)
	{
		...
		{
			const Ref<StateManager>& stateManager = mStateManagers[pipelineConstantSlot->GetProgramType()];
	...
	void CommandList::SetBlendingConstants(const RgbaColor<float>& blendingConstants)
	{
	...
	Ref<ComputeStateManager> CommandList::GetComputeStateManager() const
	{
		return ComputeStateManager::Cast(mStateManagers[Renderer::ShaderProgramType::Compute]);
	}

	Ref<GraphicStateManager> CommandList::GetGraphicStateManager() const
	{
		return GraphicStateManager::Cast(mStateManagers[Renderer::ShaderProgramType::Graphic]);
	}
	...
	void CommandList::FlushGraphicState()
	{
		...
		Ref<GraphicStateManager> graphicStateManager = GetGraphicStateManager();
	...
	{
		const Ref<StateManager>& stateManager = mStateManagers[programType];

		if (stateManager->IsDirty(StateManager::DirtyFlagBits::Pipeline))
		{
			Ref<Pipeline> pipeline = stateManager->CompilePipeline();
			Ref<Pipeline> currentPipeline = mCurrentPipelines[programType];
			if (currentPipeline != pipeline)
			{
				if (currentPipeline)
				{
					const Ref<PipelineLayout>& pipelineLayout = pipeline->GetLayout();
					const Ref<PipelineLayout>& currentPipelineLayout = currentPipeline->GetLayout();
					
					unsigned int setCount = std::min(pipelineLayout->GetDescriptorSetLayoutCount(), currentPipelineLayout->GetDescriptorSetLayoutCount());
					for (unsigned int set = 0; set < setCount; set++)
					{
						const Ref<DescriptorSetLayout>& setLayout = pipelineLayout->GetDescriptorSetLayout(set);
						const Ref<DescriptorSetLayout>& currentSetLayout = currentPipelineLayout->GetDescriptorSetLayout(set);

						if (!setLayout->IsCompatibleWith(currentSetLayout))
							mStateManagers[programType]->ResetBindings(set);
					}
				}
		...
		if (stateManager->IsDirty(StateManager::DirtyFlagBits::PipelineConstant))
		{
			for (Renderer::ShaderType shaderType : Enum<Renderer::ShaderType>::Values)
			{
				if (stateManager->IsDirtyPipelineConstants(shaderType))
				{
