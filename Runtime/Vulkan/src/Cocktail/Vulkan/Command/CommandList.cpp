...
namespace Ck::Vulkan
{
	...
	CommandList::CommandList(Ref<RenderDevice> renderDevice, Ref<CommandListPool> allocator, const Ref<DescriptorSetAllocator>& descriptorSetAllocator, const Renderer::CommandListCreateInfo& createInfo, const VkAllocationCallbacks* allocationCallbacks) :
	...
	void CommandList::EndRenderPass()
	{
		...
		if (mCurrentFramebuffer->GetSamples() != Renderer::RasterizationSamples::e1 && !hasResolveExtension)
		{
			...
			if (depthStencilAttachment)
			{
				...
				{
					mRenderDevice->Invoke([&](Ref<DepthResolver> depthStencilResolver) {
						depthStencilResolver->Resolve(*this, mCurrentFramebuffer->GetDepthStencilMultisampleAttachment(), depthStencilAttachment, depthResolveMode);
					});
				}
			}
		}
	...
	void CommandList::BindTexture(Renderer::UniformSlot* slot, unsigned int arrayIndex, const Renderer::TextureView* textureView)
	{
		...
		if (auto descriptorSetSlot = DescriptorSetSlot::TryCast(slot))
		{
			unsigned int set = descriptorSetSlot->GetSet();
			unsigned int binding = descriptorSetSlot->GetBinding();
			const Ref<StateManager>& stateManager = mStateManagers[descriptorSetSlot->GetProgramType()];

			if (slot->GetDescriptorType() == Renderer::DescriptorType::Texture)
			{
				stateManager->BindTexture(set, binding, arrayIndex, TextureView::Cast(textureView));
			}
			else if (slot->GetDescriptorType() == Renderer::DescriptorType::StorageTexture)
			{
				stateManager->BindStorageTexture(set, binding, arrayIndex, TextureView::Cast(textureView));
			}
		}
	}
	...
	void CommandList::BindUniformBuffer(Renderer::UniformSlot* slot, unsigned int arrayIndex, const Renderer::Buffer* uniformBuffer, std::size_t offset, std::size_t range)
	{
		...
		if (auto descriptorSetSlot = DescriptorSetSlot::TryCast(slot))
		{
			...
			{
				assert(slot->GetDescriptorType() == Renderer::DescriptorType::StorageBuffer);
