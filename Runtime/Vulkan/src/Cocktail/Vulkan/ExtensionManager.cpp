...
#define CASE_FEATURE(__Feature)                                                      \
		...
		CASE_FEATURE(RenderDeviceFeature::MemoryPriority)
...
#undef CASE_FEATURE
		...
		for (unsigned int i = 0; i < extensionCount; i++)
			mInstanceExtensions.emplace_back(extensionNames[i]);

		auto last = std::unique(mInstanceExtensions.begin(), mInstanceExtensions.end(), [](const char* lhs, const char* rhs) {
			return std::strcmp(lhs, rhs) == 0;
		});
		mInstanceExtensions.erase(last, mInstanceExtensions.end());
...
#undef CASE_FEATURE
		...
		for (unsigned int i = 0; i < extensionCount; i++)
			mDeviceExtensions.emplace_back(extensionNames[i]);

		auto last = std::unique(mDeviceExtensions.begin(), mDeviceExtensions.end());
		mDeviceExtensions.erase(last, mDeviceExtensions.end());
...
#undef CASE_EXTENSION
	...
	bool ExtensionManager::CheckExtensionsInstanceSupport(unsigned int extensionCount, const char* const* extensionNames)
	{
		unsigned int propertyCount;
	...
	bool ExtensionManager::CheckExtensionPhysicalDeviceSupport(VkPhysicalDevice physicalDevice, unsigned int extensionCount, const char* const* extensionNames)
	{
		unsigned int propertyCount;
	...
	bool ExtensionManager::CheckExtensionSupport(const std::vector<VkExtensionProperties>& properties, unsigned int extensionCount, const char* const* extensionNames)
	{
