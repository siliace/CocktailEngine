...
namespace Ck::Vulkan
{
	...
	Ref<ComputePipeline> PipelineCache::CreateComputePipeline(const ComputePipelineCreateInfo& createInfo)
	{
		...
		Ref<Pipeline> pipeline = mCache.GetOrCreate(stateHash, [&] {
			return mComputePipelinePool.Allocate(mRenderDevice, Self(), createInfo, mAllocationCallbacks);
		});

		return ComputePipeline::Cast(pipeline);
	}
	...
	Ref<GraphicPipeline> PipelineCache::CreateGraphicPipeline(const GraphicPipelineCreateInfo& createInfo)
	{
		...
		Ref<Pipeline> pipeline = mCache.GetOrCreate(stateHash, [&] {
			return mGraphicPipelinePool.Allocate(mRenderDevice, Self(), createInfo, mAllocationCallbacks);
		});

		return GraphicPipeline::Cast(pipeline);
	}
	...
	Ref<Renderer::RenderDevice> PipelineCache::GetRenderDevice() const
	{
		return mRenderDevice;
	}

	std::vector<unsigned char> PipelineCache::GetCacheData() const
	{
		std::size_t cacheDataSize;
		COCKTAIL_VK_CHECK(vkGetPipelineCacheData(mRenderDevice->GetHandle(), mHandle, &cacheDataSize, nullptr));

		std::vector<unsigned char> cacheData(cacheDataSize);
		COCKTAIL_VK_CHECK(vkGetPipelineCacheData(mRenderDevice->GetHandle(), mHandle, &cacheDataSize, cacheData.data()));
