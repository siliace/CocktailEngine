...
namespace Ck::D3D12
{
    ...
    public:
        ...
        void BindTexture(const Renderer::UniformSlot* slot, unsigned int arrayIndex, const Renderer::TextureView* textureView) override;
        void BindBuffer(const Renderer::UniformSlot* slot, unsigned int arrayIndex, const Renderer::Buffer* uniformBuffer, std::size_t offset, std::size_t range) override;
        ...
        void SetShadingRate(Extent2D<unsigned int> fragmentSize, Renderer::ShadingRateCombiner pipelineCombineOp, Renderer::ShadingRateCombiner primitiveCombineOp) override;
