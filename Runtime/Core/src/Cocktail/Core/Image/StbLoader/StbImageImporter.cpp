...
namespace Ck
{
	...
	Ref<Image> StbImageImporter::LoadFromPath(const std::filesystem::path& path, const ImageImportParameters& parameters) const
	{
	...
	Ref<Image> StbImageImporter::LoadFromStream(const Ref<InputStream>& inputStream, const ImageImportParameters& parameters) const
	{
		void* pixels;
		DataType dataType;
		...
		bool isHdr = stbi_is_hdr_from_callbacks(&mCallbacks, inputStream.Get()) == 1;
		inputStream->Rewind();

		if (!isHdr)
		{
			bool is16bits = stbi_is_16_bit_from_callbacks(&mCallbacks, inputStream.Get()) == 1;
			inputStream->Rewind();
			...
			if (is16bits)
			{
				...
				pixels = stbi_load_16_from_callbacks(&mCallbacks, inputStream.Get(), &width, &height, &fileChannels, desiredChannels);
			}
			else
			{
				dataType = DataType::UnsignedInt8;
				pixels = stbi_load_from_callbacks(&mCallbacks, inputStream.Get(), &width, &height, &fileChannels, desiredChannels);
			}
		}
		else
		{
		...
		Ref<Image> image = Image::New(size, pixelFormat, static_cast<const Uint8*>(pixels));
	...
	Ref<Image> StbImageImporter::LoadFromMemory(const void* buffer, std::size_t length, const ImageImportParameters& parameters) const
	{
