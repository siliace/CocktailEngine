...
namespace Ck
{
    ...
    namespace Detail::Win32
    {
        namespace
        {
            int ToWin32Type(Thread::Priority priority)
            {
                switch (priority)
                {
                    ...
                    case Thread::Priority::SlightlyBelowNormal: return THREAD_PRIORITY_NORMAL;

                    case Thread::Priority::AboveNormal: return THREAD_PRIORITY_ABOVE_NORMAL;

                    case Thread::Priority::BelowNormal: return THREAD_PRIORITY_BELOW_NORMAL;

                    case Thread::Priority::Highest: return THREAD_PRIORITY_HIGHEST;

                    case Thread::Priority::Lowest: return THREAD_PRIORITY_LOWEST;

                    case Thread::Priority::TimeCritical: return THREAD_PRIORITY_TIME_CRITICAL;
                }
        ...
        void Thread::SetAffinity(unsigned int affinityMask)
        {
            ...
            {
                CK_LOG(Detail::Win32::Win32LogCategory, LogLevel::Warning, CK_TEXT("Thread affinity mask %d must be a subset of process affinity mask %d"), affinityMask,
                       processAffinityMask);
        ...
        {
#ifdef COCKTAIL_COMPILER_GCC
            /// TODO: all of this is an ugly hack because MinGW does not support SetThreadDescription
            ...
            static UniquePtr<std::remove_pointer_t<HMODULE>, ModuleDeleter> module(GetModuleHandleA("kernelbase.dll"));
            if (module)
            {
                auto SetThreadDescription = (HRESULT(WINAPI*)(HANDLE, PCWSTR)) GetProcAddress(module.Get(), "SetThreadDescription");
