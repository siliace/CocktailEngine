...
namespace Ck::Detail::Win32
{
	namespace
	{
		LRESULT CALLBACK MainWindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
		{
			...
			if (window)
			{
				if (msg == WM_SETCURSOR)
				{
					if (Ref<Cursor> cursor = window->GetCursor())
	...
	Ref<Ck::ImageCursor> WindowFactory::CreateCursor(const CursorCreateInfo& createInfo)
	{
		return ImageCursor::New(createInfo);
	}
	...
	Ref<WindowEventQueue> WindowFactory::CreateWindowEventQueue()
	{
		return nullptr;
	}
	...
	Ref<Ck::SystemCursor> WindowFactory::LoadSystemCursor(SystemCursorType type)
	{
		return mSystemCursors.GetOrCreate(type, [&]() {
			return SystemCursor::New(type);
		});
	}

	WindowEventQueue* WindowFactory::GetMainEventQueue() const
	{
		return nullptr;
	}
}
