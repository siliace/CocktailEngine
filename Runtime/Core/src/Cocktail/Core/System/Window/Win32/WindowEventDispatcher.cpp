...
namespace Ck::Detail::Win32
{
        ...
        switch (msg)
        {
            ...
            case WM_NCLBUTTONDOWN:
            {
                switch (wParam)
                {
                    ...
                    case HTBOTTOM:
                    case HTBOTTOMLEFT:
                    case HTBOTTOMRIGHT: mResizing = true;
                        break;

                    default: mResizing = false;
                        break;
                }
            }
            break;
            ...
            case WM_ENTERSIZEMOVE:
            {
                if (mResizing)
                {
                    WindowResizingStartEvent event = CreateWindowEvent<WindowResizingStartEvent>(window);
                    window->OnResizingStartEvent().Emit(event);
                }
            }
            break;
            ...
            case WM_SIZING:
            {
                if (mResizing)
                {
                    ...
                    WindowResizingEvent event = CreateWindowEvent<WindowResizingEvent>(window);
                    event.Size.Width = AbsoluteDifference(rect->right, rect->left);
                    event.Size.Height = AbsoluteDifference(rect->bottom, rect->top);

                    window->OnResizingEvent().Emit(event);
                }
            }
            break;
            ...
            case WM_EXITSIZEMOVE:
            {
                if (mResizing)
                {
                    WindowResizingEndEvent event = CreateWindowEvent<WindowResizingEndEvent>(window);
                    window->OnResizingEndEvent().Emit(event);
                }
            }
            break;
            ...
            case WM_SIZE:
            {
                ...
                switch (wParam)
                {
                    case SIZE_MAXIMIZED: event.DisplayMode = WindowDisplayMode::Maximized;
                        break;

                    case SIZE_MINIMIZED: event.DisplayMode = WindowDisplayMode::Minimized;
                        break;

                    default: event.DisplayMode = WindowDisplayMode::Normal;
                        break;
                }
    ...
    MouseButtonEvent::ClickType WindowEventDispatcher::GetMessageClickType(UINT msg)
    {
        switch (msg)
        {
            ...
            case WM_MBUTTONDOWN:
            case WM_RBUTTONDOWN:
            case WM_XBUTTONDOWN: return MouseButtonEvent::ClickType::Pressed;

            case WM_LBUTTONUP:
            case WM_MBUTTONUP:
            case WM_RBUTTONUP:
            case WM_XBUTTONUP: return MouseButtonEvent::ClickType::Released;

            case WM_LBUTTONDBLCLK:
            case WM_MBUTTONDBLCLK:
            case WM_RBUTTONDBLCLK:
            case WM_XBUTTONDBLCLK: return MouseButtonEvent::ClickType::Double;
        }
