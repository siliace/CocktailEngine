...
namespace Ck::Detail::Win32
{
	...
	BOOL CALLBACK MonitorService::EnumMonitorCallback(HMONITOR handle, HDC deviceContext, LPRECT area, LPARAM userParam)
	{
		...
		if (GetMonitorInfo(handle, &monitorInfo) == FALSE)
			throw SystemError::GetLastError();

		const bool isPrimary = monitorInfo.dwFlags == MONITORINFOF_PRIMARY;
		Ref<Monitor> monitor = Monitor::New(handle, area, monitorInfo.szDevice, isPrimary);

		self->mMonitors.push_back(monitor);
		if (isPrimary)
			self->mPrimaryMonitor = std::move(monitor);
