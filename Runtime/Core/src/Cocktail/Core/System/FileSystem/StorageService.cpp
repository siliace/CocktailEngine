...
namespace Ck
{
	...
	void StorageService::Copy(const std::filesystem::path& source, const std::filesystem::path& destination, bool failIfExists) const
	{
		...
		Ref<FileSystemDriver> sourceDriver = ResolveProtocol(sourceProtocol);
		Ref<FileSystemDriver> destinationDriver = ResolveProtocol(destinationProtocol);
		...
		else
		{
			...
			Ref<InputStream> inputStream = FileInputStream::New(sourceFile);
			Ref<OutputStream> outputStream = FileOutputStream::New(destinationFile);
	...
	void StorageService::Mount(std::string protocol, Ref<FileSystemDriver> fileSystemDriver)
	{
		mDrivers.insert_or_assign(std::move(protocol), std::move(fileSystemDriver));
	}
	...
	Ref<FileSystemDriver> StorageService::ResolveProtocol(const std::string& protocol) const
	{
		if (protocol.empty())
