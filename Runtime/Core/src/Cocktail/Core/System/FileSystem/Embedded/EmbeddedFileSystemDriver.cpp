...
namespace Ck
{
	...
	std::unique_ptr<File> EmbeddedFileSystemDriver::OpenFile(const std::filesystem::path& path, const FileOpenFlags& flags)
	{
		for (const cmrc::embedded_filesystem& fileSystem : mFileSystems)
		{
			if (fileSystem.is_file(path.string()))
	...
	std::unique_ptr<Directory> EmbeddedFileSystemDriver::OpenDirectory(const std::filesystem::path& path)
	{
