...
namespace Ck::Detail::Win32
{
	...
	void WindowsServiceProvider::DoRegister(const Ref<Application>& application)
	{
		...
		application->Singleton<Ck::KeyboardService>([](const Ref<Application>& app) -> Ref<Ck::KeyboardService> {
			return KeyboardService::New(app);
		});
		...
		application->Singleton<Ck::MouseService>([](const Ref<Application>& app) -> Ref<Ck::MouseService> {
			return MouseService::New(app);
		});
