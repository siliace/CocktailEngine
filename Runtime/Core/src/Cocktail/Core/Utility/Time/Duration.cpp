...
namespace Ck
{
	...
	{
		Uint64 seconds = AbsoluteDifference(lhs.GetSeconds(), rhs.GetSeconds());
		Uint64 nanoseconds = AbsoluteDifference(lhs.GetNanoseconds(), rhs.GetNanoseconds());

		if (nanoseconds % 1'000'000'000 == 0)
		{
			Uint64 nanosecondsAsSeconds = TimeUnit::Seconds().ConvertFrom(seconds, TimeUnit::Nanoseconds());
		...
		if (nanoseconds % 1'000'000 == 0)
		{
			...
			Uint64 nanosecondsAsMilliseconds = TimeUnit::Milliseconds().ConvertFrom(seconds, TimeUnit::Nanoseconds());
		...
		if (nanoseconds % 1'000 == 0)
		{
			...
			Uint64 nanosecondsAsMicroseconds = TimeUnit::Microseconds().ConvertFrom(seconds, TimeUnit::Nanoseconds());
