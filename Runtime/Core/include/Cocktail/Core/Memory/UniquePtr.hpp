...
namespace Ck
{
    ...
    {
        T* pointer = allocator.Allocate(size);
        for (Uint64 i = 0; i < size; i++)
            new (pointer[i]) std::remove_extent_t<T>();

        return UniquePtr<T, AllocatorAwareDeleter<std::remove_extent_t<T>, TAllocator>>(
            pointer,
            AllocatorAwareDeleter<std::remove_extent_t<T>, TAllocator>(allocator)
        );
    }
}
