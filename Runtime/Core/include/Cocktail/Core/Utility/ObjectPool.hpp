	...
	namespace Detail
	{
		...
		public:
			...
			explicit ObjectPoolRefCounter(ObjectPoolBase<T, Lockable>* owner, Args&&... args) :
				StorageRefCounter(std::forward<Args>(args)...),
				mOwner(owner)
			{
				/// Nothing
			}
		...
		public:
			...
			explicit ObjectPoolBase(std::size_t preAllocated = 0, bool growable = true) :
				mGrowable(growable)
			{
				if (preAllocated == 0 && !mGrowable)
					throw std::invalid_argument("Object pool is pre-allocated with zero objects and is not allowed to grow");
		...
		private:
			...
			SignalOfType<Lockable, ObjectPoolBase<T, Lockable>*, std::size_t> mOnGrow;
		};
	}
