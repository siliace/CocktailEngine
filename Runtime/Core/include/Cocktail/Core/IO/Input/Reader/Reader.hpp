...
namespace Ck
{
	 ...
	 */
	class Reader : public Implements<Reader, Interface>
	{
	public:
         ...
         * Reading the reader might not totally fill the full \p length of the \p buffer if the reader does not contains enough data.
         * After reading, the reader's reading position must be advanced by the amount of bytes actually read.
		 * \param buffer A buffer where copy data read from the reader
