...
namespace Ck::Detail
{
    ...
    public:
         ...
         * \param forget Flag to indicate if the signal could be disconnected when the connection is destroyed
         * \param groupId The groupId identifier used to order Slot invocation
         * \return A connection used to manage the Slot
         */
        template <typename Callable>
        Connection Connect(Callable&& callable, bool forget = false, unsigned int groupId = 0)
        {
            // The real slot type
            using SlotType = CallableSlot<Callable, Args...>;

            // Create the slot into the signal
            Ref<Slot<Args...>> slot = CreateSlot<SlotType, Callable>(std::forward<Callable>(callable), groupId);

            // Create a connection to the slot we created
        	return Connection(slot, forget);
        }

        /**
         * \brief Connect a callable function member with compatible arguments
         * \param function The function member
         * \param object The instance of the object owning the member
         * \param forget Flag to indicate if the signal could be disconnected when the connection is destroyed
         * \param groupId The groupId identifier used to order Slot invocation
         * \return A connection used to manage the Slot
         */
        template <typename Function, typename Object>
        Connection Connect(Function&& function, Object&& object, bool forget = false, unsigned int groupId = 0)
        {
            // The real slot type
            using SlotType = FunctionMemberSlot<Function, Object, Args...>;

            // Create the slot into the signal
            auto slot = CreateSlot<SlotType, Function, Object>(std::forward<Function>(function), std::forward<Object>(object), groupId);
    ...
    private:
        ...
        Ref<Slot<Args...>> CreateSlot(SlotArgs&&... args, unsigned int groupId)
        {
            Ref<Slot<Args...>> slot = T::New(std::forward<SlotArgs>(args)..., this, groupId);

            {
				std::lock_guard<std::mutex> lg(mSlotLock);
        ...
        Lockable mSlotLock;
        std::multimap<unsigned int, Ref<Slot<Args...>>> mSlots;
    };
}
