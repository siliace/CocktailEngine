...
namespace Ck
{
	 ...
	 * \brief 
	 */
	class COCKTAIL_CORE_API StorageService : public Extends<StorageService, Object>
	{
	public:
		 ...
		 */
		void Mount(std::string protocol, Ref<FileSystemDriver> fileSystemDriver);
	...
	private:
		 ...
		 * \return 
		 */
		Ref<FileSystemDriver> ResolveProtocol(const std::string& protocol) const;

		std::string mDefaultProtocol;
		std::unordered_map<std::string, Ref<FileSystemDriver>> mDrivers;
	};
}
