...
namespace Ck
{
    ...
    struct WindowCreateInfo
    {
	    ...
	    /**
		 * \brief Specifies the event queue where process events of the Window to create
		 * If this parameter is nullptr, the events of the created Window will be processed in the current thread
		 * \see Window::PollEvents
    	 * \see WindowEventQueue::Attach
		 */
		const WindowEventQueue* EventQueue = nullptr;
    };
}
