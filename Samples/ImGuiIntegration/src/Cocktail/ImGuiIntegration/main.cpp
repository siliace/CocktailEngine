...
public:
	...
	ImGuiRenderer(Ck::Renderer::RenderDevice& renderDevice, Ck::Renderer::RenderContext& renderContext)
	{
		...
		mMaterialProgram = Ck::MaterialProgram::New("ImGui", renderDevice.CreateShaderProgram(shaderProgramCreateInfo));

		ImGuiIO& io = ImGui::GetIO();
		mFontView = CreateFontTextureView(renderContext, io);
		mFontSampler = renderDevice.CreateSampler({});

		mVertexLayout = Ck::VertexLayout::New();
		mVertexLayout->CreateAttribute(Ck::VertexAttributeSemantic::Position, Ck::DataType::Float32, 2);
		mVertexLayout->CreateAttribute(Ck::VertexAttributeSemantic::TexCoord, Ck::DataType::Float32, 2);
		mVertexLayout->CreateAttribute(Ck::VertexAttributeSemantic::Color, Ck::DataType::UnsignedInt8, 4, 1, true);
	}
