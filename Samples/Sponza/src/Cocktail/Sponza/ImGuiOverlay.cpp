...
ImGuiOverlay::ImGuiOverlay(Ck::Window& window, Ck::SceneViewer& sceneViewer, Ck::Renderer::RenderDevice& renderDevice, Ck::Renderer::RenderContext& renderContext) :
	mWindow(&window),
...
void ImGuiOverlay::Update()
{
	...
	ImGuiMouseCursor mouseCursor = io.MouseDrawCursor ? ImGuiMouseCursor_None : ImGui::GetMouseCursor();
		...
		Ref<SystemCursor> cursor = App::Resolve<WindowFactory>()->LoadSystemCursor(cursorType);
		mWindow->SetCursor(std::move(cursor));
