...
Main::ExitCode ApplicationMain(Ck::Application* application)
{
	...
	Ref<DirectionalLight> directionalLight = DirectionalLight::Create(
		scene, 
		LinearColor(1.f, 1.f, 1.f),
		Vector3<float>::Normalize(Vector3<float>::Down() - Vector3<float>::Right())
	);

	Ref<PointLight> pointLight = PointLight::Create(
		scene,
		LinearColor(0.f, 1.f, 1.f),
		Vector3<float>::Down(),
		Vector3<float>(0, 3, 0)
	);
