...
Main::ExitCode ApplicationMain(Ref<Application> application)
{
	...
	//scene->SetAmbientLight(RgbColor<float>(0.1f, 0.1f, 0.1f));

	application->Invoke([&](Ref<SceneLoader> sceneLoader) {
		sceneLoader->LoadFromPath("./resources/sponza.obj", {})->AddToScene(scene);
	});

	Vector2<float> zBounds(0.1f, 4500.f);
	Ref<PerspectiveCamera> camera = PerspectiveCamera::Create(scene,Angle<float>::Degree(60.f), (float)windowSize.Width / (float)windowSize.Height, zBounds);
